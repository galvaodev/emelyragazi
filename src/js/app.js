   ...
   $(document).ready(function() {
    ...
    $("form[name='form-contato']").submit(function( event ) {
        ...
        $.ajax({
            ...
            error: function(res){
                ...
                $('form[name="form-contato"] .msg_resposta').html('<p>Ocorreu um erro ao enviar sua mensagem.<br />Entre em contato com o administrador do sistema.</p>');
    ...
    $(window).scroll(function() {
        ...
        */
        ...
        $('[data-item]').on('click', function() {
            ...
            $(this).find('span').addClass('active-item');
        });
        ...
        */
        ...
        $(".toggle button").click(function(e) {
            ...
            $('.toggle button').find('.fa-plus').css('display', 'block');
            $('.toggle button').find('.fa-times').css('display', 'none');
            $('.toggle button').find('.btn-toggle').removeClass('btn-toggle-active');

            content.fadeToggle("fast", function() {
                if (content.is(":visible")) {
                    $(link).find('.fa-plus').css('display', 'none');
                    $(link).find('.fa-times').css('display', 'block');
                    $(link).find('.btn-toggle').addClass('btn-toggle-active');
                } else {
                    $(link).find('.fa-plus').css('display', 'block');
                    $(link).find('.fa-times').css('display', 'none');
                    $(link).find('.btn-toggle').removeClass('btn-toggle-active');
                }
            });
     ...
     $(document).ready(function () {
        ...
        $('a[href^="#"]').on('click', function (e) {
            ...
            $('html, body').stop().animate({
                ...
                $(document).on("scroll", onScroll, onScrollmb);
            });
    ...
    function onScrollmb(event){
        var scrollPosition = $(document).scrollTop();
        $('#headermb ul li a').each(function () {
            var currLink = $(this);
            var refElement = $(currLink.attr("href"));
            if (refElement.position().top -200 <= scrollPosition && refElement.position().top + refElement.height() > scrollPosition) {
               $('#headermb ul li a').removeClass("nav-active");
               currLink.addClass("nav-active");
           }
           else{
            currLink.removeClass("nav-active");
